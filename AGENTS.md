...
家庭自托管面板，三模块：米家设备控制（局域网直控）、Uptime Kuma 监控、日用品消耗/购买预测。中文 UI 与注释。

## 技术栈约束

- Python 3.12（代码用 `X | None` 语法）。FastAPI + aiosqlite（**无 ORM，全裸 SQL**）。python-miio 走局域网直控，不经云、不经 HA。
- 前端单页 HTML + vanilla JS，**无构建步骤**。不要引入前端框架/打包工具。
- 依赖只有 `requirements.txt` 里 6 个，不轻易新增。
