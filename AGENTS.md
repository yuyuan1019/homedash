...
家庭自托管面板。  
**已实现模块**：日用品库存/预测、重点待办、SMTP 周报、AI 工作台、面板登录与用户管理、Docker。  
...
4. **无密钥入库**：diff 中无 SMTP 授权码 / LLM Key / Brave Key
...
|----------|------|
| `.env` | 含密码/Key |
| `data/*session*`、真实 `*.db` 内容、`data/llm_config.json`、`data/notify_config.json`、`data/brave_config.json` | 凭据/隐私 |
| 文档/compose 中的本机绝对路径、真实邮箱密码 | 用占位符 |

- 只改 `.env.example` 的**变量名与注释示例**  
- 日志/报错/README **不得**打印完整 token  
- AI 工作台 prompt **不得**注入 `.env`、SMTP 密码
...
- **前端**：改 `app/static/*`；Tab/文案中文；风格跟随现有浅色 CSS 变量  
...
3. 「明确不做」整节 **禁止实现**（如 AI 一期不准裸 SQL）  
4. 做完后在 DEVPLAN 该待办顶部把状态改为已完成，或移入「已完成基线」，并改 README 状态表  
5. 需要用户密钥才能测的（SMTP/LLM/Brave）：代码 + dry-run/自检要能在无密钥时**优雅跳过**，不能 import 即崩
...
| **面板 AI 工作台** | 终端用户在浏览器里用自然语言改库存/待办的产品功能 |
...
- 依赖见 `requirements.txt`，不轻易新增。邮件走 stdlib smtplib；AI 用 httpx；**禁止**默认镜像内置本地大模型。
- 产品 AI：白名单 actions 写库，**禁止 LLM 直接 SQL**。
...
- 当前：`HOMEDASH_PORT`  
- 已用/可选：`SMTP_*` `NOTIFY_*` `LLM_*` `AI_*` `BRAVE_API_KEY` `AMAP_API_KEY` `AGENT_API_TOKEN` `HOMEDASH_PUBLIC_URL`
...
- 高德地图：`data/amap_config.json`（Web 服务 Key，旅游推荐交通时长）
