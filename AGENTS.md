...
- 周报邮件（待办 6）、重点待办+agent 接口（待办 8）、**AI 工作台**（待办 7：自然语言 → 白名单写库存/待办，非裸 SQL）**仅有规格、未开发**。
  - HomeDash **不内置**微信/QQ 协议；IM 由 Hermes/home agent 调 agent 接口。
  - AI **不**直接执行 SQL；只产出 JSON actions，服务端执行器写库。
