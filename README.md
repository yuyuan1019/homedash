...
- **规划中**（未实现，见 `DEVPLAN.md`）：
  ...
  - 待办 8：**重点待办** + **home agent 接口**（QQ/微信提醒由 agent 投递）
  - 待办 6：**Gmail 周报** = 重点待办 + 需购买
  - 待办 7：**AI 工作台** — 自然语言（可语音）经 LLM 生成白名单动作，**确认后写库**（库存 + 待办等）；**禁止 LLM 直接 SQL**
- 购物清单汇总
- 现在就可以先录入日用品；待办/邮件/AI 上线后数据可继续沿用
- **集成约定**：微信/QQ 由外部 home agent 发送；AI 只通过 HomeDash 动作总线改业务表
