...
- **规划中**（未实现，见 `DEVPLAN.md`）：
  - 待办 0：两口·120㎡ **EWMA + 安全库存**
  - 待办 8：**重点待办事项**（家庭 to-do）+ **home agent 接口**（`/api/agent/todos/*`，可调度到 QQ/微信提醒）
  - 待办 6：**Gmail 每周邮件** = 重点待办 + 需购买/库存摘要
  - 待办 7：**语音/一句话 + LLM 解析** → 新增或修改库存（确认后写入）
- 购物清单汇总
- 现在就可以先录入日用品；待办/邮件/语音上线后数据可继续沿用
- **集成约定**：微信/QQ 消息由外部 home agent 发送，HomeDash 只提供待办数据与 `remind-fired` 回写接口
