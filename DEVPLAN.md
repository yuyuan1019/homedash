...
**禁止**进入 prompt：`.env`、SMTP 密码、LLM key。
...
不要把本地真实路径、token、账号写进公开文档。

---

## 待办 18：旅游 — 行程规划与地点管理 — **⬜ 规划中**

**目标：** 把「旅游计划」从「目的地推荐 + 行李清单」扩展为完整的「行程规划」。用户可在每个行程内规划具体要去的地点（手动输入，或从 AI 玩法建议中选中加入），地点携带高德定位、支持一键跳转手机高德导航、顺路排序、附近美食推荐。

**子项：**

1. **行程地点数据模型**：新增 `travel_plan_spots` 表（plan_id、名称、高德经纬度 location、来源=手动/AI建议、类型=景点/美食、sort_order、备注）。手动输入的地点调高德地理编码搜索确定坐标；从 AI 玩法建议选中的，保存其高德定位。
2. **推荐玩法交互重构**：`/travel/plans/{id}/spots` 生成的非网红玩法清单，前端从「勾选画横线」改为「选中 → 加入行程规划」。**只有用户加入的地点写入 `travel_plan_spots`**；未选中的不落库，刷新后不保留。
3. **一键跳转手机高德**：每个行程地点提供「导航」按钮，生成高德地图 URI（`amapuri://` 或 universal links），移动端点击直接唤起高德导航。
4. **顺路排序**：行程详情提供「顺路排序」按钮，调用 LLM + 高德（距离矩阵/路径规划）对当日地点排最优到访顺序；手动输入的地点先经高德搜索定位再参与排序；无高德 Key 时降级 LLM 估算并明确标注。
5. **附近美食推荐**：路线规划完成后一键调 LLM + 高德 POI 周边搜索，推荐路线附近的优质美食，用户可选中加入行程规划。

**验收：**
- `python -m app.modules.travel` 自检通过（含 `travel_plan_spots` 建表、排序/美食/地理编码函数用例）。
- 手动输入地点 → 高德搜索定位成功；未配高德 Key 时优雅降级（中文提示，不 500）。
- AI 玩法建议选中 → 写入 `travel_plan_spots` 带定位；未选中刷新后消失。
- 「导航」按钮生成有效高德 URI。
- 顺路排序：多地点返回合理顺序（无高德时降级标注）。
- 美食推荐：返回带定位的候选，可加入规划。

**明确不做：** 不接美团/大众点评 API（用 LLM + 高德 POI 估算）；不做实时导航（只生成跳转 URI）；不做跨多日的复杂排程算法（仅单日内顺路排序）。

**依赖：** 高德 Key（已可选支持）；本待办是待办 19「基于行程规划细化行李清单」的前置。

---

## 待办 19：旅游 — LLM 体验与 UI 打磨 — **⬜ 规划中**

**目标：** 改善旅游模块 LLM 调用的健壮性（超时）与体验（思考过程可见、分步产出），以及旅游 UI 的可用性（卡片折叠、输入框美化）。

**子项：**

1. **LLM 调用统一加超时**：travel 模块所有 LLM 调用（推荐目的地、玩法、行李清单、顺路排序、美食推荐）统一设置超时（复用 `LLM_TIMEOUT_SEC`，默认 30s，可按场景调整），超时返回中文提示而非挂死。
2. **推荐目的地显示推理过程 + 分步调用**：`/travel/suggest` 不再一次返回全部候选；改为先返回策略/思路（思考过程），再逐个产出候选目的地；前端显示「思考中…」并逐条浮现，缓解长时间空白。
3. **行李清单显示思考过程 + 基于行程规划细化**：`/travel/plans/{id}/recommend` 先输出思考过程（目的地/天气/天数/行程地点），再输出清单；若该行程已有 `travel_plan_spots`（待办 18），结合具体活动生成更细致清单（如行程含登山则加护膝、含海边则加防晒）。
4. **我的行程卡片化 + 折叠记忆**：行程列表改为卡片，每张卡片可折叠/展开「行李清单」「行程规划」「推荐玩法」三块，展开状态用 localStorage 记忆，刷新后恢复。
5. **行李清单详情 UI 美化**：输入框样式优化，「手动添加一项 / 取消 / 保存清单」按钮重新布局，解决拥挤。

**验收：**
- 所有 LLM 调用有超时，超时不挂死、返回中文提示。
- 推荐目的地/行李清单显示「思考过程」，分步产出，不再长时间空白。
- 行程有 `travel_plan_spots` 时行李清单明显更细致（含地点相关物品）。
- 行程卡片折叠/展开状态刷新后保持。
- 行李清单详情页按钮不再拥挤。
...
| 顺序 | 待办 | 原因 |
|------|------|------|
| 1 | 18 旅游行程规划与地点管理 | 核心新功能；待办 19 的「细化行李清单」依赖它 |
| 2 | 19 旅游 LLM 体验与 UI 打磨 | 可与 18 并行（UI/超时部分先行）；「细化」子项等 18 落地 |
| 按需 | Hermes / 带 skill 的 AI 查询 due → QQ/微信 | HomeDash 接口就绪后配，不属于 HomeDash 主动推送 |
